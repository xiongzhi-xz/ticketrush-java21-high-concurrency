...
为了让项目能被验证，我补了 k6 压测、Virtual Threads vs 固定线程池 benchmark、Prometheus/Grafana 指标证据、Seata AT 示例、Elasticsearch 活动/票档读模型和本地演示页。演示时可以直接打开 http://localhost:8080/ 走初始化库存、抢票、重复提交幂等、检索和 benchmark。
```
...
1. 打开 `TicketRush 高并发抢票演示`，先看首屏核心证明点：防超卖、幂等、限流、虚拟线程、异步削峰。
2. 顶部状态为 `UP` 后，说明 Java 21、Virtual Threads、MySQL、Redis、RocketMQ、ES 都在本地 Compose 环境里。
3. 点 `1 初始化库存`：
...
5. 库存扣减方案选 `Redis Lua 原子扣减`，点 `2 发起抢票`。
...
7. 点 `3 重复提交验证幂等`，确认右侧显示重复请求被拦截，不会重复扣库存。
8. 讲主链路：
...
9. 切到 `虚拟线程压测对比`，先跑 `Java 21 虚拟线程`，再改成 `传统固定线程池` 做对比。
10. 切到 `票档检索读模型`，用 smoke 数据：
...
11. 点 `重建活动索引` 后点 `查询票档`，展示 Elasticsearch 是读模型，不参与抢票写链路。
12. 打开 Prometheus/Grafana 链接，说明指标证据和压测报告在 docs 中。
