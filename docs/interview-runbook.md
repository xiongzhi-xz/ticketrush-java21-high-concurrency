# TicketRush 面试演示 Runbook

这份文档用于明天快速熟悉 TicketRush。目标是能把一个高并发票务秒杀系统讲成可运行、可压测、可解释的工程闭环。
...
为了让项目能被验证，我补了 k6 压测、Virtual Threads vs 固定线程池 benchmark、Prometheus/Grafana 指标证据、Seata AT 示例、Elasticsearch 活动/票档读模型和本地 Demo Console。面试时可以直接打开 http://localhost:8080/ 走健康检查、库存预热、抢票、检索和 benchmark。
```
...
- Demo Console 是演示控制台，不是完整后台管理系统。
