...
```text
热门票档库存 1000
请求 A 抢票成功，Redis Lua 原子预占后库存 999
请求 B 使用新的 requestId，但复用同一个幂等 Key
重复请求返回 A0429，库存仍然 999，异步下单不重复触发
```

核心结论：

- **防超卖**：库存轨迹是 `1000 -> 999 -> 999`，重复请求不会再次扣减。
- **幂等**：`requestId A -> requestId B`，但同一个 `idempotentKey` 被识别为同一笔业务。
- **削峰**：抢票入口只做库存预占，成功后通过 RocketMQ 异步下单。
- **Java 21 落地**：抢票处理返回 `processedByVirtualThread=true`，并有执行器 benchmark 对比报告。
...
| 热点分摊 | `SKU_SPREAD=20` 时本轮 0 次 `C0429`，受理数提升到 7,496 | [docs/hotspot-spread-benchmark-report.md](docs/hotspot-spread-benchmark-report.md) |
| Virtual Threads | 纯 I/O 等待场景吞吐约为传统固定线程池的 22.55 倍 | [docs/executor-benchmark-report.md](docs/executor-benchmark-report.md) |
| Prometheus 指标 | 单热点压测 RPS 峰值 828.63/s，HTTP p95 约 3.1ms | [docs/observability-benchmark-report.md](docs/observability-benchmark-report.md) |

## GitHub 浏览顺序

如果你是第一次打开这个仓库，建议先按这个顺序看：

1. 本页的“后端工程展示”“架构图”“核心链路”“快速启动”“验证状态”。
...
截图为 `1440x900` 桌面横屏视口，覆盖本地一键抢票演示、库存 `1000 -> 999 -> 999`、用新 requestId 重复提交同一个幂等 Key、Elasticsearch 查询和执行器 benchmark 步骤。
...
启动完成后优先跑这条脚本，它会用真实接口验证防超卖、幂等和 Virtual Thread：
...
```text
health -> preload inventory 1000 -> rush request A -> repeat request B with same idempotentKey
```
...
- `.\scripts\demo-smoke.ps1`：已覆盖库存 `1000 -> 999 -> 999`、重复请求 `A0429`、同一幂等 Key 拦截和 `processedByVirtualThread=true`。
- `/`：本地辅助演示页可访问，用于需要可视化时串联核心链路、检索和执行器 benchmark。
